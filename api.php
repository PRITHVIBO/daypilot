...
try {
    ...
    if ($action === 'task_update' && $method === 'POST') {
        $in=input_json();$id=(string)($in['id']??'');$allowed=['title','description','priority','due_at','start_at','end_at','estimated_minutes','status'];$sets=[];$args=[];foreach($allowed as $f){if(array_key_exists($f,$in)){ $sets[]=$f.'=?';$args[]= $f==='priority'?normalize_priority((string)$in[$f]):($f==='estimated_minutes'?(int)$in[$f]:($f==='status'?((string)$in[$f]):($f==='title'?clean_text((string)$in[$f]):(($in[$f]??null)===null?null:date_sql((string)$in[$f]))))); }}
        if(isset($in['title'])){$idx=array_search('title=?',$sets,true);if($idx!==false)$args[$idx]=clean_text((string)$in['title']);}
        if(isset($in['description'])){$idx=array_search('description=?',$sets,true);if($idx!==false)$args[$idx]=(string)$in['description'];}
        if(!$sets)json_response(['error'=>'No fields to update.'],422);$sets[]='updated_at=?';$args[]=now();$args[]=$id;$args[]=$uid;$st=$pdo->prepare('UPDATE tasks SET '.implode(',',$sets).' WHERE id=? AND user_id=?');$st->execute($args);if($st->rowCount()===0)json_response(['error'=>'Task not found or unchanged.'],404);log_activity('update_task','task',$id);json_response(['task'=>get_task($pdo,$uid,$id)]);
    }
    ...
    if ($action === 'reminder_create' && $method === 'POST') { $in=input_json();$when=date_sql((string)($in['remind_at']??''));if(!$when||strtotime($when)<time()-60)json_response(['error'=>'Reminder time must be in the future.'],422);$id=uuid();$st=$pdo->prepare('INSERT INTO reminders(id,user_id,task_id,event_id,title,remind_at) VALUES(?,?,?,?,?,?)');$st->execute([$id,$uid,$in['task_id']??null,$in['event_id']??null,clean_text((string)($in['title']??'Reminder')),$when]);log_activity('create_reminder','reminder',$id);json_response(['ok'=>true,'id'=>$id]); }
    ...
    if ($action === 'sync' && $method === 'POST') {
        $in=input_json();$ops=$in['operations']??[];if(!is_array($ops))json_response(['error'=>'Invalid sync operations.'],422);$applied=[];$conflicts=[];
        foreach($ops as $op){ if(!is_array($op)||($op['entity']??'')!=='task')continue; $row=$op['data']??[];$id=(string)($row['id']??'');if(!$id)continue; $serverSt=$pdo->prepare('SELECT updated_at FROM tasks WHERE id=? AND user_id=?');$serverSt->execute([$id,$uid]);$server=$serverSt->fetch();$localTs=date_sql((string)($row['updated_at']??now()));
            if(($op['type']??'upsert')==='delete'){ if(!$server || !$localTs || strtotime($localTs)>=strtotime($server['updated_at'])){$del=$pdo->prepare('DELETE FROM tasks WHERE id=? AND user_id=?');$del->execute([$id,$uid]);$applied[]=$id;}else$conflicts[]=$id;continue; }
            if($server && $localTs && strtotime($localTs)<strtotime($server['updated_at'])){$conflicts[]=$id;continue;}
            $exists=(bool)$server; $t=$localTs ?: now();
            if($exists){$st=$pdo->prepare('UPDATE tasks SET title=?,description=?,status=?,priority=?,due_at=?,start_at=?,end_at=?,estimated_minutes=?,completed_at=?,updated_at=? WHERE id=? AND user_id=?');$st->execute([clean_text((string)($row['title']??'')), $row['description']??null, in_array(($row['status']??'open'),['open','done','archived'],true)?$row['status']:'open',normalize_priority((string)($row['priority']??'medium')),date_sql($row['due_at']??null),date_sql($row['start_at']??null),date_sql($row['end_at']??null),isset($row['estimated_minutes'])?(int)$row['estimated_minutes']:null,date_sql($row['completed_at']??null),$t,$id,$uid]);}
            else{$st=$pdo->prepare('INSERT INTO tasks(id,user_id,title,description,status,priority,due_at,start_at,end_at,estimated_minutes,completed_at,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)');$st->execute([$id,$uid,clean_text((string)($row['title']??'')),$row['description']??null,$row['status']??'open',normalize_priority((string)($row['priority']??'medium')),date_sql($row['due_at']??null),date_sql($row['start_at']??null),date_sql($row['end_at']??null),isset($row['estimated_minutes'])?(int)$row['estimated_minutes']:null,date_sql($row['completed_at']??null),$t,$t]);}
            $applied[]=$id;
        }
        json_response(['ok'=>true,'applied'=>$applied,'conflicts'=>$conflicts,'server_tasks'=>fetch_tasks($uid,'open',200)]);
    }

    if ($action === 'ai_chat' && $method === 'POST') { $in=input_json();$msg=trim((string)($in['message']??''));if($msg==='')json_response(['error'=>'Message is required.'],422);$result=gemini_chat($msg,$u);json_response($result); }
